...
| Stability | Deterministic team IDs + normalized names + season‑aware feature store |
| Monitoring | Global & per‑team drift, anomaly detection, calibration, conference aggregation |
| Lineage | Config version + git commit embedded in every primary CSV |
| Safety Nets | Row inflation guard, feature alignment logic, low‑data game exclusion |
...
python3 daily_pipeline.py                 # Run full scrape → feature store → train → predict → update
python3 game_prediction/view_predictions.py  # Optional: CLI viewer
```

Minimal one‑liner (fresh predictions only, assuming data present):
```bash
...
```
ESPN / ncaahoopR → Raw Games → Normalization → Feature Store (rolling diffs) ┐
                       │               │
                     Training Dataset       │
                       │               │
...
Safeguards:
* De‑dup by `game_id` always.
* Season boundary logic prevents mixing prior season rolling stats.
* Low‑data game filter excludes matchups where either team lacks minimum historical depth.
